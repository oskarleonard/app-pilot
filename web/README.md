# app-pilot (web) — agent eyes & hands on your live web app (web engine)
...
the run). `app-pilot shot` archival screenshots are headless regardless.
...
- **`scripts/app-pilot/app-pilot`** = everything around it: the pinned tester server (your
  own dev server is never touched), the compaction-proof run journal + audit
  log, archival full-page screenshots, the ground-truth sweep, and app-pilot-assets
...
- Money/destructive flows: per the product rails (`product/RUNBOOK.md`);
  universal minimum — every confirm is logged (`app-pilot act`) + screenshotted.
